...
- Calls your `AgentProcessor.process` with the final transcript; emits its `send` events back to the socket (e.g., `complete` with `responseText`).
- Streams TTS via `SpeechProvider.stream`, wrapping binary audio with `tts.start` / `tts.end`.
