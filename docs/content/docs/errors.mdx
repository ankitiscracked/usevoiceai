...
- Wire shape: `type: "session.error"`, `data: { code: VoiceErrorCode; message: string; retryable?: boolean; details?: Record<string, unknown>; error?: string }`
...
`SpeechStream` rejects its iterator when `tts.end` arrives with `errored`. Catch it in your player:
