...
- **A command is already in progress**: Wait for `tts.end`/`complete`, or call `cancelRecording()` before starting again.
...
- **Auto mode never completes**: Your STT provider isn’t emitting `speech-end` hints; keep using manual mode or enable VAD/endpointing via `speechEndDetection.options`.
